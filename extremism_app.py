...
Переключайтесь и сравниваете результаты - кажеться чуть лучще rubert
Но нужно пересмотреть на разных примерах

1. **distiluse** (DistilUSE):
   - Легкая и быстрая модель
   - Поддерживает множество языков
   - Хорошо работает для общих задач

2. **rubert-base-cased-sentence**:
   - Специализирована для русского языка
   - Лучше понимает контекст и семантику русских текстов
   - Может быть более точной для специфических русских выражений


Выберите модель в зависимости от ваших конкретных потребностей и характера анализируемых текстов.
""")

# Объяснение Порога максимального сходства
st.sidebar.markdown("""
## Что такое Порог максимального сходства?

Порог максимального сходства - это значение, которое определяет, насколько похожим должно быть сообщение на известные экстремистские категории, чтобы быть помеченным как потенциально экстремистское.

- Значение ближе к 0: более строгое обнаружение, больше сообщений может быть помечено как экстремистские.
- Значение ближе к 1: менее строгое обнаружение, только очень похожие сообщения будут помечены как экстремистские.

Настройте этот параметр для баланса между чувствительностью обнаружения и количеством ложных срабатываний.
""")
...
extremist_keywords = {
    ...
    "russophobia": ["русофобия", "кацапы", "москали", "ватник", "вата",
                    ...
                    "лапти", "лаптеногие", "мокшане", "угрофины",
